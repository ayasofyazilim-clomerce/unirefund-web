...
export default async function Layout({children, params}: {children: React.ReactNode; params: {lang: string}}) {
  ...
  return (
    <div className="h-screen overflow-hidden">
      ...
      <main className="h-[calc(100vh-4rem)] overflow-y-auto bg-white p-0 [&:not(:has(.mobile-fullscreen))]:p-6 md:[&:not(:has(.mobile-fullscreen))]:px-8 md:[&:not(:has(.mobile-fullscreen))]:py-6">
        {children}
      </main>
    </div>
  );
