...
> {
  ...
  > = {
    [key]: {
      description:
        languageData[item.description as keyof typeof languageData] ||
        item.description ||
        "",
      displayName:
  ...
  if (
    ...
    config[key].fieldType = convertValueTypeNameToFieldType(
      item.valueType.name as AllowedValueTypeModelNameStringEnum,
    );
  }
  return config;
          ...
          return {
            [key]: {
              ...
              description: description(
                languageData[
                  subitem.description as keyof typeof languageData
                ] ||
                  subitem.description ||
                  "",
              ),
...
function createBonds(sett: CreateBondType): BondType[] {
  return sett.bonds.map((bond) => {
    const sourceField = sett.parentField
