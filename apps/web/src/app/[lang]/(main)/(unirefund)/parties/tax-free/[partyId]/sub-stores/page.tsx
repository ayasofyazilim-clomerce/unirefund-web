...
import { getTaxFreeSubStoresByIdApi } from "src/actions/unirefund/CrmService/actions";
import ErrorComponent from "src/app/[lang]/(main)/_components/error-component";
import { getResourceData } from "src/language-data/unirefund/CRMService";
import SubStoresTable from "./table";

async function getApiRequests(
  filters: GetApiCrmServiceTaxFreesByIdSubTaxFreeData,
) {
  try {
    const session = await auth();
    const apiRequests = await Promise.all([
      getTaxFreeSubStoresByIdApi(filters, session),
    ]);
    return {
