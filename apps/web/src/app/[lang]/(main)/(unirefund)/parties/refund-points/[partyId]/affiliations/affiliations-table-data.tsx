...
import {deleteRefundPointsByIdAffiliationsByAffiliationIdApi} from "@repo/actions/unirefund/CrmService/delete-actions";
import {
          ...
          content={{
            ...
            action: (
              <Button asChild className="text-blue-500" variant="link">
                <Link href={getBaseLink("settings/affiliations/refund-points")}>{languageData.New}</Link>
              </Button>
            ),
          }}>
...
function affiliationsRowActions(
  ...
  if (isActionGranted(["CRMService.RefundPoints.DeleteAffiliation"], grantedPolicies)) {
    actions.push({
      type: "confirmation-dialog",
      ...
      icon: Trash,
      onConfirm: (row) => {
        void deleteRefundPointsByIdAffiliationsByAffiliationIdApi({
          id: partyId,
