...
import {deleteTaxOfficesByIdAffiliationsByAffiliationIdApi} from "@repo/actions/unirefund/CrmService/delete-actions";
import {
...
function affiliationsTableActions(
  ...
  if (isActionGranted(["CRMService.TaxOffices.CreateAffiliation"], grantedPolicies)) {
          ...
          content={{
            ...
            action: (
              <Button asChild className="text-blue-500" variant="link">
                <Link href={getBaseLink("settings/affiliations/tax-offices")}>{languageData.New}</Link>
              </Button>
            ),
          }}>
    ...
    actions.push({
      ...
      icon: Plus,
      onClick() {
        router.push(getBaseLink(`parties/individuals/new?entityPartyTypeCode=TAXOFFICE&partyId=${partyId}`));
      },
    });
  }
  return actions;
...
function affiliationsRowActions(
  ...
  if (isActionGranted(["CRMService.TaxOffices.DeleteAffiliation"], grantedPolicies)) {
    actions.push({
      type: "confirmation-dialog",
      ...
      icon: Trash,
      onConfirm: (row) => {
        void deleteTaxOfficesByIdAffiliationsByAffiliationIdApi({
          id: partyId,
