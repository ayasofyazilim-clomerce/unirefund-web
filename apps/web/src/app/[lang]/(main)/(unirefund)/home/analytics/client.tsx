...
import {issuedTagsByRefundMethod, issuedTagsByTimeOfDay, topChains} from "./data";

const formatCurrency = (value: unknown) => `${Intl.NumberFormat("tr").format(Number(value)).toString()}₺`;

export type IssuedTagsByTimeOfWeekday = {
  label: string;
  tags: number;
}[];

export type IssuingNationalities = Record<string, {label: string; value: number}>;

export default function Client({
  issuedTagsByTimeOfWeekday,
  issuingNationalities,
}: {
  issuedTagsByTimeOfWeekday: IssuedTagsByTimeOfWeekday;
  issuingNationalities: IssuingNationalities;
}) {
  return (
    <DashboardLayout
      cols={4}
