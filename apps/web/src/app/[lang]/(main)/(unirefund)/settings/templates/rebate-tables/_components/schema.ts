import { ContractServiceResource } from "@/language-data/unirefund/ContractService";
...
export function createRebateTableFormSchemas({ languageData }: { languageData: ContractServiceResource }) {
    const processingFeeDetailsSchema = z.array(
        z.object({
            name: z.string(),
            amount: z.number(),
        })
    ...
    const rebateTableDetailsSchema = z.array(
        z.object({
        ...
        if (allRows.length > 1) {
            ctx.addIssue({
                code: z.ZodIssueCode.custom,
                message: languageData["RebateTable.Form.rebateTableDetails.refundMethodOnlyOneRowAllowedWhenRefundPointIsAll"],
                path: [],
            });
        }
    });
    ...
    const createFormSchema = z.object({
        ...
        if (!data.isTemplate) {
            if (!data.merchantId) {
                ctx.addIssue({
                    code: z.ZodIssueCode.custom,
                    message: languageData["RebateTable.Form.rebateTableDetails.merchantSelectionIsRequiredWhenIsTemplateOptionFalse"],
                    path: ["merchantId"]
                });
            }
        }
