...
function findNavPath(items: NavItem[], targetPath: string, currentPath: NavItem[] = []): NavItem[] | null {
  ...
  for (const item of items) {
    ...
    const pathMatches = (itemHref && normalizedTarget.startsWith(itemHref)) || normalizedTarget.startsWith(itemKey);
...
export function SidebarLayoutProvider({
  ...
  const [breadcrumbs, setBreadcrumbs] = useState<Breadcrumb[]>(defaultBreadcrumbs || [{label: "Home", href: "/"}]);
      ...
      if (navPath) {
        ...
        if (lastNavHref) {
          ...
          if (remainingSegments.length > 0) {
            const dynamicBreadcrumbs = createBreadcrumbsFromSegments(remainingSegments, languageData, navSegments);
        ...
        const fallbackBreadcrumbs: Breadcrumb[] = [{label: "Home", href: "/"}];

        const segmentBreadcrumbs = createBreadcrumbsFromSegments(filteredSegments, languageData);
  ...
  return <SidebarLayoutContext.Provider value={contextValue}>{children}</SidebarLayoutContext.Provider>;
}
