...
test("refund point form - stable random", async ({page}) => {
  ...
  await page.goto("/en/parties/refund-points/new");
  await appReady(page);
  ...
  await fillStable(page, by("root_address_postalCode"), "020202");
  await safeClick(by("root_address_type"));
  await safeClick(by("root_address_type_WORK"));
  const prevUrl = page.url();
  await safeClick(by("create-refund-point-form_submit"));
