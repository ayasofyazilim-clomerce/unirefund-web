...
test("create tag - stable random", async ({page}) => {
  test.setTimeout(45_000);
  const unique = `${Date.now()}${Math.floor(Math.random() * 1000)}`;
  const randomName = `Tag-${unique}`;
  const randomExternalId = randDigits(9);
  const randomVatNumber = randDigits(10);

  await page.goto("/en/parties/tax-free/new");
  await appReady(page);
  ...
  await fillPhoneTR(page, by("phone"), "5442444444");
  ...
  await safeClick(by("create-tax-free-form_submit"));
